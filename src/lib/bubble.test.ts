...
function event(id: string, title: string, joinUrl: string | null = null): CalendarEvent {
  return { id, title, start: START, end: new Date(START.getTime() + 1_800_000), joinUrl };
}
...
describe('meetingBubbleContent', () => {
  it('carries the single meeting through unchanged', () => {
    ...
    expect(content.meetings).toEqual([{ title: 'Standup', joinUrl: 'https://zoom.us/j/1' }]);
  ...
  it('lists every simultaneous meeting so the user can pick', () => {
    const content = meetingBubbleContent(
      [event('a', 'Standup', 'https://zoom.us/j/1'), event('b', 'Design Review', null)],
      delta,
    );

    expect(content.title).toBe('2 meetings at once');
    expect(content.meetings).toEqual([
      { title: 'Standup', joinUrl: 'https://zoom.us/j/1' },
      { title: 'Design Review', joinUrl: null },
    ]);
  });
